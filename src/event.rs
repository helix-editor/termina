...
/// Optional metadata carried by an enhanced keyboard protocol key event.
///
/// The Kitty keyboard protocol can report the shifted key, the key in the base keyboard layout,
/// and the text generated by a key event in addition to its canonical key code.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct KeyEventEnhancements {
    /// The character that would be produced with Shift applied.
    pub shifted_key: Option<char>,

    /// The character in the base keyboard layout (without modifiers/layout translations applied).
...
impl KeyEventEnhancements {
    /// Creates a new, empty set of key event enhancements.
...
pub struct KeyEvent {
    ...
    /// Optional metadata reported by an enhanced keyboard protocol.
    pub enhancements: KeyEventEnhancements,
}
